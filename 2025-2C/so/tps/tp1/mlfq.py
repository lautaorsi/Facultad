...
while finishedJobs < totalJobs:
    ...
    for i in job:
        if job[i]['timeLeft'] > 0 and currTime >= job[i]['startTime'] and not job[i]['doingIO'] and i != currJob: 
            job[i]['waitingTime'] += 1
    ...
    if timeLeft == 0:
        ...
        if currTime % 2 == 0 and currTime != 0:
            print(f'[AGREGADO TP] THROUGHPUT: {finishedJobs/currTime}')
